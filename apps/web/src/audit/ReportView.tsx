...
// загрузки данных из sessionStorage) живёт здесь, чтобы /personal/report
// (реальный отчёт) и /example (статичный демо-отчёт без БД) рендерили ОДИН
// и тот же компонент и не могли разойтись по дизайну. См. ExampleReportPage.
// ---------------------------------------------------------------------------
...
export const ReportView: React.FC<{
  result: SubmitResult;
  isCorporateSource: boolean;
  /** /example — статичный демо-отчёт без БД: скрывает нижнюю кнопку "Пройти
   * ещё раз" (её заменяет плавающая кнопка "Попробовать" на самой странице
   * /example) и подписывает шапку как пример, а не как реально сохранённый
   * результат. */
  isExample?: boolean;
  /** Доп. блок, вставляемый сразу после карточки Inwell Score — используется
   * только на /example для мини-истории "3 замера за 6 месяцев". */
  historyStrip?: React.ReactNode;
}> = ({ result, isCorporateSource, isExample = false, historyStrip }) => {
  const { t, lang } = useLanguage();
          ...
          <div className="text-center space-y-1 sm:space-y-1.5">
            ...
            <p
              className={`text-xs font-medium ${
                isExample ? 'text-slate-500' : isCorporateSource ? 'text-emerald-600' : result.trackable ? 'text-emerald-600' : 'text-amber-600'
              }`}
            >
              {isExample ? r.exampleNote : isCorporateSource ? t.corporate.publicAudit.submittedNote : result.trackable ? r.savedNote : r.notSavedNote}
            </p>
          </div>
